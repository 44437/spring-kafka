...
public class ConsumerRepository {
  ...
  // If specific.avro.reader is set false, you need to take it with ConsumerRecord<String, GenericRecord> record and record.value().get("field_key");
  // I set it true because there is a class generated by avro, it will be used directly.
  @KafkaListener(topics = Constants.TOPIC_FIRST, groupId = Constants.CONSUMER_GROUP_ID)
  public void consumeMessageFromPartitionIMPORTANT(ConsumerRecord<String, MessageReq> record, @Header(KafkaHeaders.OFFSET) Long offset) {
    logger.info("IMPORTANT Got something: {}-{}-{}", offset, record.partition(), record.value());// record.offset() works too
    HttpStatus status = arbitraryClient.sendRequest("/", record.value().toString());
...
//  @KafkaListener(topicPartitions = @TopicPartition(topic = Constants.TOPIC_FIRST, partitions = {Constants.PARTITION_UNIMPORTANT}), groupId = Constants.CONSUMER_GROUP_ID)
  @KafkaListener(topics = Constants.TOPIC_FIRST, groupId = Constants.CONSUMER_GROUP_ID)
  public void consumeMessageFromPartitionUNIMPORTANT(ConsumerRecord<String, MessageReq> record, @Header(KafkaHeaders.OFFSET) Long offset) {
